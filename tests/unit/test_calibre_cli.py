...
# --- Kindle sidecar thumbnail helpers -----------------------------------------


def _make_minimal_mobi(uuid: str | None, cdetype: str | None) -> bytes:
    """Synthesize a MOBI file just big enough that read_mobi_identity parses
    EXTH 113 and 501 out of it. Layout: 78-byte PalmDB header (with
    type=BOOK creator=MOBI), one record-info entry pointing at the
    PalmDOC+MOBI+EXTH blob.
    """
    import struct
    ...
    # MOBI header. We only need a few fields parsed: sig, header_length
    # (at +4), exth_flag (at +0x70). Pad to make 0x74 bytes total.
    ...
    # PalmDB: 78-byte fixed header + 8 bytes per record info entry, then
    # 2 bytes of gap, then record bytes. rec0 starts at offset 88.
    rec0_off = 88
    palmdb = bytearray(b"\x00" * 78)
    # type/creator at 60-67
    palmdb[60:68] = b"BOOKMOBI"
    # num records at 76-77
    palmdb[76:78] = struct.pack(">H", 1)
    record_info = struct.pack(">I", rec0_off) + b"\x00\x00\x00\x00"  # offset + attr/uid
    gap = b"\x00\x00"
    return bytes(palmdb) + record_info + gap + rec0


def test_read_mobi_identity_extracts_uuid_and_cdetype(tmp_path):
    f = tmp_path / "fake.mobi"
    f.write_bytes(_make_minimal_mobi("test-uuid-1234", "EBOK"))
    assert calibre_cli.read_mobi_identity(f) == ("test-uuid-1234", "EBOK")


def test_read_mobi_identity_missing_cdetype_returns_none(tmp_path):
    f = tmp_path / "fake.mobi"
    f.write_bytes(_make_minimal_mobi("only-uuid", None))
    assert calibre_cli.read_mobi_identity(f) == ("only-uuid", None)


def test_read_mobi_identity_non_mobi_returns_none(tmp_path):
    f = tmp_path / "garbage.epub"
    f.write_bytes(b"PK\x03\x04" + b"\x00" * 200)
    assert calibre_cli.read_mobi_identity(f) == (None, None)


def test_read_mobi_identity_missing_file_returns_none(tmp_path):
    assert calibre_cli.read_mobi_identity(tmp_path / "nope.mobi") == (None, None)


def test_kindle_thumbnail_name_format():
    assert (
        calibre_cli.kindle_thumbnail_name("abc-123", "EBOK")
        == "thumbnail_abc-123_EBOK_portrait.jpg"
    )


def test_make_kindle_thumbnail_writes_jpeg(tmp_path):
    PIL = pytest.importorskip("PIL.Image")
    src = tmp_path / "cover.jpg"
    img = PIL.new("RGB", (200, 300), color=(64, 128, 255))
    img.save(src, "JPEG")
    dst = tmp_path / "thumb.jpg"
    assert calibre_cli.make_kindle_thumbnail(src, dst, size=(100, 142)) is True
    assert dst.is_file()
    out = PIL.open(dst)
    assert out.format == "JPEG"
    assert out.size == (100, 142)


def test_make_kindle_thumbnail_returns_false_on_bad_source(tmp_path):
    src = tmp_path / "not-an-image.jpg"
    src.write_bytes(b"definitely not jpeg")
    dst = tmp_path / "thumb.jpg"
    assert calibre_cli.make_kindle_thumbnail(src, dst) is False
